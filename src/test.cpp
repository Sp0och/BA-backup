...
void create_points_2d(std::vector<cv::Point2f>& input_vector_2d,std::vector<cv::Point2f>& normed_vector, std::vector<cv::Point3f>& input_vector_3d, int IW, int IH){
...
int main(int argc, char **argv){
    ...
    create_points_2d(points_2d,points_normed_2d, points_3d,IMAGE_WIDTH,IMAGE_HEIGHT);
    ...
    std::vector<cv::Point2f> points_2d_c(100);
    for(int i = 0; i < 100; i++)
    points_2d_c[i] = points_2d[i];
...
// std::vector<cv::Point2f> projected_2;
//     // std::vector<cv::Point2f> projected_c;
//     projected_2.resize(points_3d.size());
//     for(size_t i = 0; i<projected_2.size();i++){
//         cv::Point2f point;
//         point.x = points_3d[i].x;
//         point.y = points_3d[i].y;
//         projected_2[i] = point;
//     }
    // for(size_t i = 0; i<projected_c.size();i++){
    //     cv::Point2f point;
    //     point.x = points_3d_c[i].x;
    //     point.y = points_3d_c[i].y;
    //     projected[i] = point;
    // }
    ROS_INFO("length of 2d before is: %i",points_2d_c.size());
    ROS_INFO(" length of 2d after is: %i", points_2d.size());

    publish_matches(&publisher,test_image,points_2d_c,points_2d,1,cv::Scalar(0,255,0),cv::Scalar(0,0,255));
