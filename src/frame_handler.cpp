...
/* void frame_handler::loadVocabulary(std::string voc_path)
{
    voc = new DBoW3::Vocabulary(voc_path);
    db.setVocabulary(*voc, false, 0);
} */

void frame_handler::addFrame(Frame* cur_kf, bool flag_detect_loop)
{
    /* int loop_index = -1;
    if (flag_detect_loop)
    {
    ...
    {
        addFrameIntoVoc(cur_kf);
    } */
    int loop_index = !flag_detect_loop*(-1);

    // check loop if valid using ransan and pnp
    if (loop_index != -1)
    {
        Frame* old_kf = getFrame(loop_index);
    ...
    // add Frame
    cur_kf->freeMemory();
    Framelist.push_back(cur_kf);
    // if(Framelist.size() > 3){
    //     Framelist.pop_back();
    // }
}

Frame* frame_handler::getFrame(int index)
{
    list<Frame*>::iterator it = Framelist.begin();
...
/* int frame_handler::detectLoop(Frame* Frame, int frame_index)
{
    //first query; then add this frame into database!
    DBoW3::QueryResults ret;
    db.query(Frame->bow_descriptor_and_images, ret, 4, frame_index - MIN_LOOP_SEARCH_GAP);
    db.add(Frame->bow_descriptor_and_images); // ret[0] is the nearest neighbour's score. threshold change with neighour score

    if (DEBUG_IMAGE)
    {
        image_pool[frame_index] = Frame->image.clone();

        cv::Mat bow_images = Frame->image.clone();
...
/* void frame_handler::addFrameIntoVoc(Frame* Frame)
{
    if (DEBUG_IMAGE)
    {
        image_pool[Frame->index] = Frame->image.clone();
    }

    db.add(Frame->bow_descriptor_and_images);
} */