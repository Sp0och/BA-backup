...
class BRIEF
{
    ...
    vector<cv::Point2f> brief_keypoints_2d;
    ...
    BRIEF(const cv::Mat &_input_image, 
        ...
        if(mode == 1)
        KP_pub_intensity = n.advertise<sensor_msgs::Image>("brief_keypoints_intensity", 1);
        else if(mode == 2)
        KP_pub_range = n.advertise<sensor_msgs::Image>("brief_keypoints_range", 1);
        else
        KP_pub_ambient = n.advertise<sensor_msgs::Image>("brief_keypoints_ambient", 1);
...
/**
 * Stores the BRIEF keypoints in the vector in the std::vector format, creates the descriptors around the keypoints and calls the ransac point creator
 * @param brief_keypoints_2d is then the vector containing the keypoints
 * */
    void create_descriptors(){
        ...
        if(mode == 1)
        publish_keypoints(&KP_pub_intensity, image,brief_keypoints_2d,5,cv::Scalar(0,255,0));
        else if(mode == 2)
        publish_keypoints(&KP_pub_range, image,brief_keypoints_2d,5,cv::Scalar(0,255,0));
        else
        publish_keypoints(&KP_pub_ambient, image,brief_keypoints_2d,5,cv::Scalar(0,255,0));
    };
    ...
    void points_for_ransac(){
        brief_point_3d.resize(brief_keypoints_2d.size());
        brief_point_2d_norm.resize(brief_keypoints_2d.size());
        vector<uchar> status;
        status.resize(brief_keypoints_2d.size());
        #pragma omp parallel for num_threads(NUM_THREADS)

        for(int i = 0; i < brief_keypoints_2d.size(); i++){
            int row_index = (int)brief_keypoints_2d[i].y;
            int col_index = (int)brief_keypoints_2d[i].x;
        ...
        trimVector(brief_point_3d,status);
        trimVector(brief_point_2d_norm,status);
    }
