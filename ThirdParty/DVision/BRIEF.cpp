  ...
  m_bit_length(nbits), m_patch_size(patch_size), m_type(type)
{
  assert(patch_size > 1);
  assert(nbits > 0);
  //we generate the random test points here in the constructor
  generateTestPoints();
}
...
void BRIEF::compute(const cv::Mat &image, 
    ...
    vector<bitset> &descriptor_and_images,
    bool treat_image) const
{
  ...
  //create our image
  cv::Mat im;
  //only treat the image if wanted
  if(treat_image)
  {
    cv::Mat aux;
    if(image.depth() == 3)
    ...
    //smooth the picture before feature extraction to reduce sensitivity => increase stability and repeatability of the descriptor_and_images
  ...
  //at this point the image is in grayscale and blurred by a gaussian
  assert(im.type() == CV_8UC1);
  assert(im.isContinuous());
  
  const int W = im.cols;
  const int H = im.rows;
  //resize descriptor_and_image to the m_bit_length
  descriptor_and_images.resize(points.size());
  ...
  dit = descriptor_and_images.begin();
  for(kit = points.begin(); kit != points.end(); ++kit, ++dit)
  {
    dit->resize(m_bit_length);
    dit->reset();
    //m_x1.size() is just the the amount of testpoints -> iterate trough all the testpoints.
    for(unsigned int i = 0; i < m_x1.size(); ++i)
    {
      //set points to the keypoints + the shift of the random test points to compare values
      ...
      if(x1 >= 0 && x1 < W && y1 >= 0 && y1 < H 
        && x2 >= 0 && x2 < W && y2 >= 0 && y2 < H)
      {
        //compare the testpoints around the current keypoint and thus set the descriptor_and_image bit
...
{  
  ...
  //iterate throught the descriptor_and_image array
  for(int i = 0; i < m_bit_length; ++i)
  {
    int x1, y1, x2, y2;
    //get point x1,y1 inside the patch
    do
    {
