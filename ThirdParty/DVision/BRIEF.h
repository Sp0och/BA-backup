 ...
 *   Elementary Features) descriptor_and_image by 
 *   Michael Calonder, Vincent Lepetit and Pascal Fua
 *   + close binary tests (by Dorian Galvez-Lopez)
 *
 * If you use this code with the RANDOM_CLOSE descriptor_and_image version, please cite:
...
/// BRIEF descriptor_and_image
class BRIEF
{
public:

  /// Bitset type used to later determine hamming
  typedef boost::dynamic_bitset<> bitset;
//you use enum to restrict content of variables to certain values
  /// Type of pairs
  enum Type
  {
...
public:
  ...
  /**
   * Creates the BRIEF a priori data for descriptor_and_images of nbits length
   * @param nbits descriptor_and_image length in bits
   * @param patch_size 
   * @param type type of pairs to generate
   */
  //BRIEF constructor initializes with a patch size considered of size 48 and 256 bits for the RGB color space
  BRIEF(int nbits = 256, int patch_size = 48, Type type = RANDOM_CLOSE);
  virtual ~BRIEF();
  
  /**
   * Returns the descriptor_and_image length in bits
   * @return descriptor_and_image length in bits
   */
  inline int getdescriptor_and_imageLengthInBits() const
  {
    return m_bit_length;
  }
  ...
  /**
   * Returns the BRIEF descriptor_and_images of the given keypoints in the given image
   * @param image
   * @param points
   * @param descriptor_and_images 
  ...
  inline void operator() (const cv::Mat &image, 
    ...
    std::vector<bitset> &descriptor_and_images,
    bool treat_image = true) const
  {
    compute(image, points, descriptor_and_images, treat_image);
  }
  ...
  /**
   * Returns the Hamming distance between two descriptor_and_images
   * @param a first descriptor_and_image vector
   * @param b second descriptor_and_image vector
...
protected:
   ...
   * m_patch_size and m_bit_length so depending on the patch which can be overlayed on any image
   */
  void generateTestPoints();
  
protected:

  /// descriptor_and_image length in bits
  int m_bit_length;
